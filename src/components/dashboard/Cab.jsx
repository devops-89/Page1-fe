...
// Updated Columns for Cab Booking
const columns = [
  { key: "sl", label: "SL" },
  { key: "fullName", label: "Full Name" },
  { key: "phone", label: "Phone" },
  { key: "email", label: "Email" },
  { key: "taxiType", label: "Taxi Type" },
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", px: { xs: 2, sm: 3, md: 4 }, py: { xs: 3, sm: 4, md: 5 } }}>
      ...
      <Box mb={4} textAlign="center">
        ...
        <Typography variant="body1" sx={{ fontFamily: nunito.style, fontWeight: 400, color: COLORS.BLACK }}>
          Track and manage all your cab bookings. Use the filters below to search and browse customer rides.
        </Typography>
      </Box>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }}>
          <TableHead sx={{ backgroundColor: COLORS.PRIMARY }}>
            <TableRow>
              {columns.map((col) => (
                <TableCell key={col.key} sx={{ fontWeight: 600, color: COLORS.WHITE, textAlign: "center", fontFamily: nunito.style, whiteSpace: "nowrap" }}>
                  {col.label}
                </TableCell>
              ))}
            </TableRow>
          ...
          <TableBody>
            {paginatedData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={columns.length} align="center" sx={{ py: 3, fontFamily: nunito.style, fontWeight: 600 }}>
      ...
      {/* Pagination */}
      <Box mt={2} display="flex" flexDirection={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems="center" gap={1}>
