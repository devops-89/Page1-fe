    ...
    const departureDateHandler = (newDate, index) => {
        ...
        if (isValid && newDate) {
            if (index > 0 && forms[index - 1].departure_date) {
                ...
                if (moment(newDate).isBefore(prevDate)) {
                    dispatch(
                        setToast({
                            open: true,
                            message: "Departure date must be greater than or equal to the previous departure date.",
            ...
            setState((prevState) => {
                ...
                updatedMulticity[index].departure_date = moment(newDate._d).format("YYYY-MM-DD");
                return ({
                    ...prevState,
                    multicity: updatedMulticity,
                });
