...
const MultiTravellerSelector = ({
    ...
    useEffect(() => {
        if (router.pathname === defaultRoute && newFormData) {
            setState((prevState) => ({
                ...prevState,
                cabin_class: newFormData.cabin_class || '1', 
                adult: newFormData.adult, 
                child: newFormData.child, 
                infant: newFormData.infant, 
            }));
            setAdultValue(newFormData.adult);
            setChildValue(newFormData.child); 
            setInfantValue(newFormData.infant); 
        } else {
            setAdultValue(state.adult);
            setChildValue(state.child);
            setInfantValue(state.infant);
        }
    }, [router.pathname, defaultRoute, newFormData, setState, setAdultValue, setChildValue, setInfantValue, state.adult, state.child, state.infant]);
...
export default MultiTravellerSelector;